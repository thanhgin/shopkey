...
function phanTich_URL_chiTietSanPham() {
    var masp = window.location.href.split('?')[1]; // lấy tên
    if(!masp) return khongTimThaySanPham();
	...
	if(sanPhamHienTai.description){	s = "";
	for(let mota of sanPhamHienTai.description){
		s += addMota(mota);
	}
	}
    info.innerHTML = s;
